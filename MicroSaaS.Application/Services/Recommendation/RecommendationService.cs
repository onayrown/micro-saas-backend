...
public class RecommendationService : IRecommendationService
{
    ...
    public async Task<GrowthRecommendationDto> GenerateCustomGrowthRecommendationAsync(CustomRecommendationRequestDto request)
    {
        // Validar o request
        ...
        // Gerar recomendação personalizada com base nos parâmetros fornecidos
        var recommendation = new GrowthRecommendationDto
        {
            Id = Guid.NewGuid(),
            CreatorId = request.CreatorId,
            Title = $"Recomendação personalizada de crescimento: {request.RecommendationType}",
            Description = $"Recomendação personalizada baseada em {request.SpecificTopic ?? "tópicos gerais"} " +
                         $"para {request.TargetAudience ?? "sua audiência atual"}",
            Category = GrowthCategory.Engagement, // Default, poderia ser determinado pelo request
            Platform = request.Platform,
            ImplementationSteps = new List<string>
            {
                "Analise o desempenho atual do seu conteúdo",
                $"Adapte seu conteúdo com foco em {request.SpecificTopic ?? "tópicos relevantes"}",
                "Implemente as mudanças e monitore resultados por 14 dias",
                "Ajuste sua estratégia com base nos resultados obtidos"
            },
            ExpectedOutcome = $"Aumento de engajamento e crescimento na plataforma {request.Platform}",
